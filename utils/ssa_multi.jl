...
function generate_synthetic(construct_delay::Function, params, L1, L, I, numofrun, inter, noise::Bool, ex_cv, ids)
    ...
    [begin
         ...
         for (k, id) in enumerate(ids)
             if x[id] == 1
                 val = k   # state 4->1, 5->2, 6->3
                 break
             end
         end
         val
     end for x in ens[i].u]
    ...
    if noise
        return (syn = nascent_syn, nsyn = noise_syn, true_trace = convert.(Vector{Float64}, final_state_vector))
    else
        return (syn = nascent_syn, ens = ens, true_trace = convert.(Vector{Float64}, final_state_vector))# remove ens
    end
end
