function run_train(seed::Int,ref_data,folder_name::String,model_name::String,noise::Bool)
    ...
    # Data Preparation
    lengths = [length(vcat(ref_data[i].nsyn...)) for i in 1:3700] # change!
    ...
    x_train, y_train = x_tensor[:, :, 1001:3400], y_tensor[:, :, 1001:3400] #change!
    x_val, y_val = x_tensor[:, :, 3401:end], y_tensor[:, :, 3401:end]
