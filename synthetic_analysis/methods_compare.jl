...
@load "synthetic_data/ideal_data_res/genb_mtest.jld2" genb_mtest;
@load "synthetic_data/ideal_data_res/gennb1_mtest.jld2" gennb1_mtest;
@load "synthetic_data/ideal_data_res/gennb0_mtest.jld2" gennb0_mtest;
...
function save_data_methods(name::AbstractString, level::AbstractString)
    ...
    save_tag = Dict("nb0"=>"gennb0","nb1"=>"gennb1","b"=>"genb")[lvl]
...
save_data_methods("ma",  "nb0");

# or run all combinations note that hmm method is quite slow and can take ~3 hours, other methods take several seconds.
#=
function save_all_methods_levels()
    names  = ["hmm","ma","sg"]
    levels = ["nb0","nb1","b"]
    results = Dict{Tuple{String,String}, Any}()
    for lvl in levels, n in names
        results[(lvl,n)] = save_data_methods(n, lvl)
    end
    return results
end

res = save_all_methods_levels();  # runs all 3×3 combos
=#